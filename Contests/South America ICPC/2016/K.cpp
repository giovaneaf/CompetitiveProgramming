/*
Check each possibility of werewolf (i) going from 0 to n-1.
For each one, note that if any person have i in his list then he'll vote to kill the werewolf.
So you have the number of votes to the current werewolf called votes here.
To the werewolf win then each person that isn't in his list should receive at most votes-1 and
if it's in his wish list then should receive at most votes-2.
So you'll construct a flow network and check if it's possible to all vote (using max flow), if it's then the 
werewolf lost, if all can't vote then the werewolf is safe.
*/
